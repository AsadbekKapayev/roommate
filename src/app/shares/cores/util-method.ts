...
// import {CommonBoi} from '@app/models/commons/bo_models/CommonBoi';
...
// }
//
// export function getRouterUrl(route: ActivatedRoute): string {
...
// }
//
// export function savePreviousValue(promise: Function, value) {
...
//   return error?.error?.cause?.cause?.message ?? error.error.message ?? error.message;
// }
//
...
//   return (window.URL || window.webkitURL).createObjectURL(url);
// }
//
// export function doCardNumber(startWithDigits: string, endWithDigits: string) {
//   if (!startWithDigits || startWithDigits.length < 6) {
//     return startWithDigits + '** **** ' + endWithDigits;
//   }
//   return startWithDigits.substring(0, 4) + ' ' + startWithDigits.substring(4) + '** **** ' + endWithDigits;
// }
//
// export function observeInputEvent(element: HTMLInputElement) {
//   return fromEvent(element, 'input')
//     .pipe(
//       debounceTime(500),
//       map((event: Event) => (event.target as HTMLInputElement).value),
//       filter(value => !(value.trim().length === 1 || value.trim().length === 2)),
//       distinctUntilChanged()
//     );
// }
//
// export function onlyUniqueBoi(value: CommonBoi, index, self) {
//   return self.map(boi => boi.instanceId).indexOf(value.instanceId) === index;
// }
//
// export function setImageUrlsForHtmlImages(html: string): string {
//   const els = document.createElement('html');
//   els.innerHTML = html;
//
//   const spans = els.getElementsByTagName('span');
//   const ps = els.getElementsByTagName('p');
//
//   for (let index = 0; index < spans.length; index++) {
//     const el = spans.item(index);
//     el.setAttribute('style', '');
//   }
//
//   for (let index = 0; index < ps.length; index++) {
//     const el = ps.item(index);
//     el.setAttribute('style', '');
//   }
//
//   const imgElements = els.getElementsByTagName('img');
//
//   for (let index = 0; index < imgElements.length; index++) {
//     const element = imgElements.item(index);
//     const attr = element.getAttribute('src');
//
//     if (!attr.startsWith('[fileId:') || !attr.endsWith(']')) {
//       continue;
//     }
//
//     const fromFileId = attr.indexOf(':') + 1;
//     const lengthFileId = (attr.length - fromFileId) - 1;
//
//     const fileId = attr.substr(fromFileId, lengthFileId);
//     const newAttrValue = environment.imageUrl + '/' + fileId;
//
//     element.setAttribute('src', newAttrValue);
//   }
//
//   return els.getElementsByTagName('body').item(0).innerHTML;
// }
